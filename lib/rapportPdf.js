...
async function loadProjetForPdf(projetId) {
  return queryOne(
    `SELECT p.*, t.nom AS type_nom, t.couleur, c.nom AS commune_nom, c.region AS region_nom,
...
async function sectionBudget(doc, projet) {
  ...
  const depAtt = await queryOne(
    'SELECT COALESCE(SUM(montant),0) AS s, COUNT(*) AS c FROM depenses WHERE projet_id=? AND validee=false',
    [projet.id]
  );
