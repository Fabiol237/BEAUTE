...
const Map = dynamic(() => import('@/components/Map'), { 
  ssr: false,
  loading: () => <div style={{ height: '650px', width: '100%', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12 }}>Chargement de la carte...</div>
})
  ...
  return (
    <div>
      ...
      <div className="dashboard-layout">
        <div className="card" style={{ height: '650px', padding: 0, overflow: 'hidden' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', height: '650px' }}>
          <h3 className="mb-4">Liste des Projets ({filteredProjects.length})</h3>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {filteredProjects.map((p) => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                  ...
                  <span style={{ fontSize: '0.7rem', fontWeight: 700 }}>{p.avancement_physique}%</span>
                </div>
            ...
            {!loading && filteredProjects.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--muted)' }}>
                ...
                <button onClick={fetchProjects} className="btn btn-outline mt-2">Actualiser</button>
              </div>
            )}
          </div>
